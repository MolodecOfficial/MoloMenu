package ru.bmprojects.molomenu.client.screens.widget
...
import ru.bmprojects.molomenu.client.screens.util.Alignment
import ru.bmprojects.molomenu.client.screens.widget.layout.ILayoutConsumer

open class MoloWidget(x: Int, y: Int, width: Int, height: Int, text: Component) :
    ...
    fun betterBlit(
        ...
        blit(
                stack,
                MoloScreen.getAlignmentPosX(alignment, offsetX + this.x, width, targetWidth, size),
                MoloScreen.getAlignmentPosY(alignment, offsetY - this.y, height, targetHeight, size),
                texX.toFloat(),
                texY.toFloat(),
    ...
    open fun tick() {
        this.widgets.forEach { if (it is MoloWidget) it.tick() }
    }
}